    ...
    <main className="min-h-screen bg-[#100F0C] pb-28">
      ...
      <div className="max-w-4xl mx-auto px-6" style={{ paddingTop: 'max(calc(env(safe-area-inset-top) + 72px), 88px)', paddingBottom: 32 }}>
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            href="/briefs/new"
            className="bg-[#C6F23E] text-[#100F0C] font-semibold rounded-xl px-4 py-2.5 text-sm hover:bg-[#ADDA38] transition-colors"
          >
            + New brief
          </Link>
        </div>
        ...
        {!briefs || briefs.length === 0 ? (
          <div className="bg-[#17150F] border border-[#3a3730] rounded-xl p-12 text-center">
            ...
            <Link
              href="/briefs/new"
              className="inline-flex bg-[#C6F23E] text-[#100F0C] font-semibold rounded-xl px-4 py-2.5 text-sm hover:bg-[#ADDA38] transition-colors"
            >
              Create a brief
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {briefs.map((brief) => {
              ...
              return (
                <div
                  key={brief.id}
                  className="bg-[#17150F] border border-[#3a3730] rounded-xl p-5 hover:border-[#4a4640] transition-colors"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <Link href={`/briefs/${brief.id}`}>
                        <h2 className="text-white font-semibold hover:text-[#C6F23E] transition-colors truncate">
                          {brief.title}
                        </h2>
                      </Link>
                      <div className="flex items-center gap-4 mt-2">
                        {brief.budget && (
                          <span className="text-[#C6F23E] text-sm font-medium">
