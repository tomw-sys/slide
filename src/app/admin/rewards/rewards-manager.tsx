    ...
    <div>
      ...
      {!showNew && !editingReward && (
        <button
          ...
          className="mb-6 bg-[#C6F23E] text-[#100F0C] font-semibold rounded-xl px-4 py-2.5 text-sm hover:bg-[#ADDA38] transition-colors"
        >
          Add reward
        </button>
      )}
      ...
      {rewards.length === 0 ? (
        <div className="bg-[#17150F] border border-[#3a3730] rounded-xl p-10 text-center">
          ...
          <p className="text-[#8a8575] text-sm">Add your first reward offer above.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-4">
          {rewards.map((reward) => {
            const tierColour = TIER_COLOUR[reward.minimum_tier] ?? 'text-[#8a8575]'
            ...
            return (
              <div
                key={reward.id}
                className={`bg-[#17150F] border rounded-xl p-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between ${
                  reward.is_active ? 'border-[#3a3730]' : 'border-[#3a3730] opacity-50'
                }`}
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 flex-wrap mb-1">
                    <p className="text-white font-semibold">{reward.partner_name}</p>
                    <span className="bg-[#C6F23E]/10 text-[#C6F23E] text-xs font-bold px-2.5 py-0.5 rounded-lg">
                  ...
                  <p className="text-[#F4EFE3] text-sm mb-2">{reward.offer_description}</p>
                  <div className="flex items-center gap-4 text-xs text-[#8a8575] flex-wrap">
                    <span>
                      Code:{' '}
                      <span className="font-mono text-[#F4EFE3]">{reward.discount_code}</span>
                    </span>
                    <span>
                      Tier:{' '}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button
                    ...
                    className="text-xs font-medium border border-[#3a3730] text-[#8a8575] rounded-lg px-3 py-1.5 hover:border-[#4a4640] hover:text-white transition-colors"
                  >
                    Edit
                  </button>
